...
hard-wired to the bus's electrical system; the satellite-positioning
principles underlying such receivers are treated comprehensively by
Kaplan and Hegarty~[23]. While the vehicle moves, the
...
Participation in the fallback path is deliberately \emph{opt-in per
trip}: the phone enters arbitration only when a driver has been assigned
to the trip and is actively sharing through the driver application. A
trip scheduled without an assigned driver relies solely on the hardware
tracker. This gate serves two purposes: it is an authorization
boundary---no unassigned device can publish positions for a bus---and it
keeps normal operation entirely free of any dependency on a driver's
phone. What is automatic is therefore the \emph{selection} between the
sources that are live, not the activation of the backup source itself.
...
\begin{tikzpicture}[font=\small,
  st/.style={draw, rounded corners=3pt, fill=blue!6, minimum width=2.6cm,
             minimum height=9mm, align=center},
  lb/.style={font=\footnotesize, align=center},
  nt/.style={font=\footnotesize, text=black!60, align=center,
             text width=3.7cm},
  ar/.style={-{Latex[length=2.4mm,width=2mm]}, semithick}]
\node[st] (pl) at (0,0)     {\textsc{planned}};
\node[st] (pt) at (4.55,0)  {\textsc{pre\_trip}};
\node[st] (ru) at (9.10,0)  {\textsc{running}};
\node[st] (en) at (13.65,0) {\textsc{ended}};
\draw[ar] (pl) -- (pt);
\draw[ar] (pt) -- (ru);
\draw[ar] (ru) -- (en);
% transition labels: raised fully above the boxes, no overlap
\node[lb] at (2.275,1.05)  {scheduled job\\opens window};
\node[lb] at (6.825,1.05)  {dwell inside\\origin geofence};
\node[lb] at (11.375,1.05) {trip completed\\or closed};
% state notes: fixed width, one per box, cannot collide
\node[nt, anchor=north] at (4.55,-0.75)
  {rider sees pre-departure state: parked / approaching / arrived at origin};
\node[nt, anchor=north] at (9.10,-0.75)
...
Realtime delivery uses Socket.IO over WebSocket, the IETF-standardized
protocol that provides a persistent, full-duplex channel over a single
TCP connection~[24]. Each active trip is a logical \emph{room}; a passenger tracking a trip joins that trip's room
