...
\begin{figure}[H]
\centering
\includegraphics[width=0.31\textwidth]{driver_shots/routemap.jpg}\hfill
\includegraphics[width=0.31\textwidth]{driver_shots/source_phone.jpg}\hfill
\includegraphics[width=0.31\textwidth]{driver_shots/live.jpg}
\caption[Screens of the driver application]{The driver application.
Left to right: (a) the assigned trip's route map with the
location-source selector (bus device / my phone) and the \emph{Start
trip} control; (b) trip preparation with \emph{my phone} chosen as the
location source and the stop sequence listed---done, next, and upcoming
states, with stop names rendered in both scripts; (c) an active trip
publishing \textsc{live}: next stop (11 of 17) with its ETA, current
speed, GPS accuracy, seconds since the last update, and the \emph{End
trip} control.}
\label{fig:driver}
\end{figure}
