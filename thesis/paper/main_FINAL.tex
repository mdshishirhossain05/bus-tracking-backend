...
Fig.~\ref{fig:architecture} shows the overall architecture, and
Fig.~\ref{fig:dataflow} follows one position report on its journey from
the bus to a passenger's screen.
...
\node[blk, fill=blue!8, text width=68mm, anchor=north]
  ...
  {\textbf{UniBus application server}\\[-1pt]
   ...
   {\tiny (the step-by-step journey is shown in Fig.~\ref{fig:dataflow})}};
...
\end{tikzpicture}
\caption{Overall architecture of UniBus Live. The bus position comes
mainly from a fixed GPS tracker that works with no driver involvement;
the driver's phone is only an automatic backup. A single low-cost
cloud server receives, checks, and enriches the data, then delivers
live updates to the passenger app and the admin website.}
...
\end{tikzpicture}}%
\caption{How one position report travels through the system, from the
GPS fix on the bus to the passenger's screen. Steps 3--5 are detailed
in Sections~\ref{sec:impl-arbitration}--\ref{sec:impl-eta}.}
\label{fig:dataflow}
\end{figure}
