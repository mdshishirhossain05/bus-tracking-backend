...
Fig.~\ref{fig:architecture} details the architecture and data flow.
...
% ================= VPS ZONE =================
\node[zone, fit=(trac)(srv), inner sep=2.8mm] (vps) {};
...
% ================= LEGEND =================
\node[anchor=north, font=\tiny, align=left] at ($(czone.south)+(0,-1.6mm)$)
  {\tikz{\draw[arr] (0,0) -- (4.5mm,0);}\, live telemetry / realtime push\qquad
   \tikz{\draw[rarr] (0,0) -- (4.5mm,0);}\, REST command / query};
\end{tikzpicture}}%
\caption{Detailed UniBus Live system architecture and data flow. The
hard-wired GT06S tracker is the autonomous primary source (no driver
device or action); the driver phone is an optional REST fallback. The
server's five-stage pipeline, backed by PostgreSQL and Redis, pushes
per-trip state to clients over Socket.IO rooms.}
