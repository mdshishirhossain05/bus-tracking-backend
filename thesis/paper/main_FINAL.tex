...
Tracking System with Reliable Dual-Source GPS Acquisition}

\author{
  \IEEEauthorblockN{Md.\ Shishir Hossain, Md.\ Sajib Hasan, Md.\ Showkat Hossen}
  \IEEEauthorblockA{\textit{Department of Computer Science and Engineering} \\
  \textit{NPI University of Bangladesh} \\
  Manikganj, Dhaka, Bangladesh\\
  \{mdshishirhossain3\}@gmail.com\\[0.6em]}
...
was 5.9~s. These results demonstrate the feasibility of reliable,
...
from a generic vehicle tracker. First, \emph{reliability}: positions come primarily from a fixed hardware
...
\begin{itemize}
  ...
  health-arbitrated smartphone fallback, providing reliable coverage at
  low cost.
...
\emph{deployed}, low-cost system emphasizing source reliability, a
...
The server is implemented in TypeScript on Node.js~\cite{nodejs} using the
Express framework~\cite{express}. Persistence is provided by
PostgreSQL~\cite{postgres} through the Prisma~\cite{prisma}
...
passenger, and audit and operations. An in-memory Redis~\cite{redis} instance serves two
...
Realtime delivery uses Socket.IO~\cite{socketio} over the WebSocket
protocol~\cite{rfc6455}, with each active trip represented as a
...
the bus---the satellite-positioning principles underlying such receivers are
treated comprehensively in~\cite{kaplan}---reporting to a self-hosted Traccar
telematics server. An optional
...
The passenger application is built with React Native~\cite{reactnative} and
Expo~\cite{expo}, rendering live vehicle and route geometry on a map. It uses a foreground location service
...
console is a server-rendered React (Next.js)~\cite{nextjs} application integrating the
...
freshest healthy source. Fallback participation is opt-in per trip---the
phone enters arbitration only when a driver is assigned to the trip and is
actively publishing---so what is automatic is the \emph{selection} among live
sources, while trips scheduled without a driver rely on the tracker alone. Every accepted fix is also appended to a per-trip
...
distinguished by reliable dual-source GPS acquisition, a lightweight
...
\bibitem{nodejs}
OpenJS Foundation, ``Node.js.'' \url{https://nodejs.org/}
\bibitem{express}
OpenJS Foundation, ``Express web framework for Node.js.''
\url{https://expressjs.com/}
\bibitem{prisma}
Prisma Data, Inc., ``Prisma ORM.'' \url{https://www.prisma.io/}
\bibitem{postgres}
PostgreSQL Global Development Group, ``PostgreSQL.''
\url{https://www.postgresql.org/}
\bibitem{redis}
Redis Ltd., ``Redis.'' \url{https://redis.io/}
\bibitem{socketio}
Socket.IO, ``Socket.IO: bidirectional and low-latency communication.''
\url{https://socket.io/}
\bibitem{reactnative}
Meta Platforms, Inc., ``React Native.'' \url{https://reactnative.dev/}
\bibitem{expo}
Expo, ``Expo: universal native apps with React.'' \url{https://expo.dev/}
\bibitem{nextjs}
Vercel, Inc., ``Next.js: the React framework.'' \url{https://nextjs.org/}
\bibitem{kaplan}
E.~D. Kaplan and C.~J. Hegarty, Eds., \textit{Understanding GPS/GNSS:
Principles and Applications}, 3rd~ed. Norwood, MA, USA: Artech House, 2017.
\bibitem{rfc6455}
