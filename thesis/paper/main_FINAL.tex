...
Figs.~\ref{fig:sources}--\ref{fig:deliver} walk through the
architecture piece by piece: where the bus position comes from, how it
reaches the server, what the server does with each report, and who
sees the result.
\suppressfloats[t]%
...
\end{tikzpicture}
\caption{Where the bus position comes from. The fixed GPS tracker is the
main source and needs no driver; the driver's phone is only an
automatic backup.}
\label{fig:sources}
\end{figure}
...
\end{tikzpicture}}
\caption{How position reports travel from the bus to the UniBus server.}
\label{fig:ingest}
\end{figure}
...
\end{tikzpicture}}
\caption{What the server does with every incoming report (details in
...
\end{tikzpicture}
\caption{Who receives the live data.}
\label{fig:deliver}
\end{figure}
