...
\node[stp] (s1) at (0,0)
  {\stepnum{1} \textbf{GPS fix captured on the bus}\\[-0.5pt]
   {\tiny\color{black!65} captured every 5\,s while the bus is moving}};
\node[stp] (s2) at (0,-11.2)
  {\stepnum{2} \textbf{Transmitted and decoded}\\[-0.5pt]
   {\tiny\color{black!65} sent over the cellular network; decoded in $\approx$1\,s}};
\node[stp] (s3) at (0,-22.4)
  {\stepnum{3} \textbf{Best source selected}\\[-0.5pt]
   {\tiny\color{black!65} tracker preferred; phone covers tracker outages}};
\node[stp] (s4) at (0,-33.6)
  {\stepnum{4} \textbf{Faulty readings rejected}\\[-0.5pt]
   {\tiny\color{black!65} inaccurate fixes, impossible jumps and jitter removed}};
\node[stp] (s5) at (0,-44.8)
  {\stepnum{5} \textbf{Arrival times calculated}\\[-0.5pt]
   {\tiny\color{black!65} remaining distance $\div$ recent speed, per stop ahead}};
\node[stp] (s6) at (0,-56.0)
  {\stepnum{6} \textbf{Stored and published}\\[-0.5pt]
   {\tiny\color{black!65} saved in PostgreSQL; published through Redis}};
\node[stp] (s7) at (0,-67.2)
  {\stepnum{7} \textbf{Pushed to every subscribed phone}\\[-0.5pt]
   {\tiny\color{black!65} WebSocket delivers position and new arrival times}};
\node[stp, fill=green!11, draw=green!45!black!45] (res) at (0,-78.4)
